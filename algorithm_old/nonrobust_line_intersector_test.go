...
func TestLineIntersectionPointOnLine(t *testing.T) {
	for _, tc := range []struct {
		...
		{
			p: geom.Coord{0, 0}, p1: geom.Coord{-1, -1}, p2: geom.Coord{1, 1}, result: POINT_INTERSECTION,
		},
		{
			p: geom.Coord{-1, -1}, p1: geom.Coord{-1, -1}, p2: geom.Coord{1, 1}, result: POINT_INTERSECTION,
		},
		{
			p: geom.Coord{1, 1}, p1: geom.Coord{-1, -1}, p2: geom.Coord{1, 1}, result: POINT_INTERSECTION,
		},
		...
		if tc.result != intersector.Result {
			t.Errorf("%v != intersector.CalculateIntersectionPointOnLine(%v, %v, %v)", tc.result, tc.p, tc.p1, tc.p2)
		} else {
			hasIntersection := tc.result == POINT_INTERSECTION
			if hasIntersection != intersector.HasIntersection() {
				t.Errorf("%v != intersector.HasIntersection()", hasIntersection, tc.p, tc.p1, tc.p2)
			}
		}
	}
}
