...
func ComputeEdgeDistance(p, p0, p1 geom.Coord) (float64, error) {
	dx := math.Abs(p1[0] - p0[0])
	dy := math.Abs(p1[1] - p0[1])

	dist := -1.0 // sentinel value
	if p.Equal(geom.XY, p0) {
		dist = 0.0
	} else if p.Equal(geom.XY, p1) {
		if dx > dy {
			dist = dx
		} else {
			dist = dy
		...
		pdx := math.Abs(p[0] - p0[0])
		pdy := math.Abs(p[1] - p0[1])
		if dx > dy {
			dist = pdx
		} else {
			dist = pdy
		...
		if dist == 0.0 && !p.Equal(geom.XY, p0) {
			dist = math.Max(pdx, pdy)
		}
	}
	if dist == 0.0 && !p.Equal(geom.XY, p0) {
		return 0, fmt.Errorf("Bad distance calculation, dist = %v, p=%v, p0=%v", dist, p, p0)
	}
	return dist, nil
}

type LineIntersectionStrategy interface {
	computeIntersection(p, p1, p2 geom.Coord) (isProper bool, result Intersection)
...
func CalculateIntersectionPointOnLine(layout geom.Layout, strategy LineIntersectionStrategy, p, p1, p2 geom.Coord) *LineIntersector {
	intersector := newLineIntersector(layout, strategy, p1, p2)
	intersector.isProper, intersector.Result = strategy.computeIntersection(p, p1, p2)
	return intersector
}
