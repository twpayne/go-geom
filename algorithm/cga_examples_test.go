...
func ExampleOrientationIndex() {
	...
	orientation := algorithm.OrientationIndex(vectorOrigin, vectorEnd, target)
...
func ExampleIsPointInRing() {
	...
	inRing := algorithm.IsPointInRing(ring.Layout(), geom.Coord{1, 1}, ring.FlatCoords())
	fmt.Println(inRing)
	// Output: false
}
...
func ExampleLocatePointInRing() {
	ring := geom.NewLinearRingFlat(geom.XY, []float64{10, 10, 20, 10, 30, 30, 10, 30, 10, 10})
	pointInRing := algorithm.LocatePointInRing(ring.Layout(), geom.Coord{15, 15}, ring.FlatCoords())
...
func ExampleIsOnLine() {
	...
	onLine := algorithm.IsOnLine(line.Layout(), geom.Coord{5, 0}, line.FlatCoords())
	fmt.Println(onLine)
	// Output: true
}
...
func ExampleIsRingCounterClockwise() {
	ring := geom.NewLinearRingFlat(geom.XY, []float64{10, 10, 20, 10, 30, 30, 10, 30, 10, 10})
	clockwise := algorithm.IsRingCounterClockwise(ring.Layout(), ring.FlatCoords())
...
func ExampleDistanceFromPointToLine() {
	...
	distance := algorithm.DistanceFromPointToLine(p, lineStart, lineEnd)
...
func ExamplePerpendicularDistanceFromPointToLine() {
	...
	lineEnd := geom.Coord{10, 10}
	distance := algorithm.PerpendicularDistanceFromPointToLine(p, lineStart, lineEnd)
	fmt.Println(distance)
	// Output: 10
}
...
func ExampleDistanceFromPointToLineString() {
	...
	distance := algorithm.DistanceFromPointToLineString(lineString.Layout(), p, lineString.FlatCoords())
	fmt.Println(distance)
	// Output: 40
}
...
func ExampleDistanceFromLineToLine() {
	...
	distance := algorithm.DistanceFromLineToLine(line1.Coord(0), line1.Coord(1), line2.Coord(0), line2.Coord(1))
	fmt.Println(distance)
	// Output: 10
}
...
func ExampleSignedArea() {
	ring := geom.NewLinearRingFlat(geom.XY, []float64{10, 10, 20, 10, 30, 30, 10, 30, 10, 10})
	singedArea := algorithm.SignedArea(ring.Layout(), ring.FlatCoords())
...
func ExampleIsPointWithinLineBounds() {
	...
	isWithinLineBounds := algorithm.IsPointWithinLineBounds(point, line.Coord(0), line.Coord(1))
...
func ExampleDoLinesOverlap() {
	...
	overlaps := algorithm.DoLinesOverlap(line1Start, line1End, line2Start, line2End)
...
func ExampleEqual() {
	...
	isEqual := algorithm.Equal(coords, 0, coords, 1)
	fmt.Println(isEqual)
	// Output: false
}
...
func ExampleDistance2D() {
	...
	distance := algorithm.Distance2D(geom.Coord(coords[0:2]), geom.Coord(coords[2:4]))
	fmt.Println(distance)
	// Output: 20
}
