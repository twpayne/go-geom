...
func TestNewRadialSorting(t *testing.T) {
	...
	for i, tc := range []struct {
		...
		pts := algorithm.NewRadialSorting(geom.XY, []float64{tc.p1[0], tc.p1[1], tc.p2[0], tc.p2[1]}, tc.origin)
		isLess := pts.Less(0, 1)
...
func TestNewRadialComparatorSorting(t *testing.T) {
	...
	sorting := algorithm.NewRadialSorting(geom.XY, pts, geom.Coord{-17.1041375307579, 42.3147318674446})
	sort.Sort(sorting)
